...
sequenceDiagram
    ...
    AUTH->>P: Request login credentials
...
sequenceDiagram
    ...
    P->>PORTAL: Request data export
    PORTAL->>SEC: Verify patient identity
    SEC->>P: Request additional verification
    P->>SEC: Provide verification
    SEC-->>PORTAL: Identity verified
    PORTAL->>HMS: Request data export
    HMS->>DB: Retrieve all patient data
    DB-->>HMS: Return complete patient record
    HMS->>EXPORT: Process data export
    EXPORT->>EXPORT: Format data (HL7 FHIR)
    EXPORT->>EXPORT: Encrypt data
    EXPORT-->>HMS: Export file ready
    HMS->>PORTAL: Provide download link
    PORTAL-->>P: Download link available
    P->>PORTAL: Download patient data
    HMS->>DB: Log data export activity
```
